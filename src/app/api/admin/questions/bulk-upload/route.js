...
export async function POST(req) {
  try {
    ...
    for (const raw of rows) {
      const course_id = raw.course_id;
      const subject = sanitizeText(raw.subject);
      const chapter = sanitizeText(raw.chapter);
      const topic = sanitizeText(raw.topic);
      const question_text = sanitizeText(raw.question_text || raw.question);
      const question_image_url = sanitizeText(raw.question_image_url);
      const option_a = sanitizeText(raw.option_a);
      const option_b = sanitizeText(raw.option_b);
      const option_c = sanitizeText(raw.option_c);
      const option_d = sanitizeText(raw.option_d);
      const option_e = sanitizeText(raw.option_e);
      const option_f = sanitizeText(raw.option_f);
      const option_g = sanitizeText(raw.option_g);
      const option_h = sanitizeText(raw.option_h);
      const correct_option = sanitizeText(raw.correct_option);
      const explanation = sanitizeText(raw.explanation);
      const image_url = sanitizeText(raw.image_url);
      const question_type = sanitizeText(raw.question_type);
