...
export async function POST(req) {
    try {
        const { chapter_ids, limit = 20, user_id, question_type } = await req.json();

        if (!user_id || !chapter_ids || !Array.isArray(chapter_ids) || chapter_ids.length === 0) {
            return NextResponse.json(
                { success: false, message: 'user_id and chapter_ids array are required' },
                { status: 400 }
            );
        }
        ...
        // 1) Candidate questions for scope (filtered by subject for integrity)
        ...
        if (qErr) throw qErr;
        const candidates = (qData || []).map((x) => x.id);

        if (candidates.length < limit) {
            return NextResponse.json(
                { success: false, error: `Only ${candidates.length} questions available. Please enter ${candidates.length} or fewer questions.` },
                { status: 400 }
            );
        }
        ...
        if (pick.length > 0) {
            ...
            // Secondary verification: ensure all fetched questions strictly belong to the requested chapters and subject
            const byId = new Map((qs || [])
                .filter(q => {
                    const matchChapter = chapter_ids.includes(String(q.chapter_id));
        ...
        const { data: attempt, error: attemptErr } = await supabase
            ...
            .insert({
                user_id,
                // Using null because there are multiple chapter IDs
                chapter_id: null,
        ...
        if (attempt?.id && pick.length > 0) {
            // First, fetch correct keys for these questions
            ...
            if (!cErr && corrects) {
                ...
                if (ansErr) {
                    console.error('Failed to persist questions to quiz_answers:', ansErr);
                }
            }
        }
        ...
        console.error('AMC Multi-Chapter Quiz Error:', err);
