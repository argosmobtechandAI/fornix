...
function ModernAudioPlayer({ src, title, color = "blue", icon = <Volume2 size={16} /> }) {
  ...
  const colorClasses = {
    blue: "bg-gradient-to-r from-orange-500 to-amber-500",
    indigo: "bg-gradient-to-r from-orange-500 to-amber-500",
        ...
        <div className="flex items-center justify-center h-8 gap-0.5 mt-2">
          {Array.from({ length: 30 }).map((_, i) => {
            ...
            return (
              <div
                key={i}
                className={`w-1 rounded-full transition-all duration-200 ${isPlaying ? `${colorClasses[color]} opacity-${Math.min(100, 20 + i * 3)}` : 'bg-gray-300 dark:bg-gray-600'
                  }`}
                style={{
...
function MediaManager({ question, onUpdated }) {
  ...
  const femaleUrl = audioUrls[selectedLanguage]?.female || (selectedLanguage === "en" ? question?.female_explanation_audio_url : null);
  const maleUrl = audioUrls[selectedLanguage]?.male || (selectedLanguage === "en" ? question?.male_explanation_audio_url : null);
    ...
    try {
      ...
      if (action === "regenExplanation") {
        ...
        const res = await fetch(`/api/admin/questions/${question.id}/voice`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ voice: params.voice, language: selectedLanguage, regenerate: params.regenerate }),
        });
        const data = await res.json();
        if (!res.ok || !data?.success) throw new Error(data?.error || "Failed to generate audio");
        
        // Locally mutate `question` so the UI instantly updates without full page reload
        if (!question.explanation_audio_urls) question.explanation_audio_urls = {};
        if (!question.explanation_audio_urls[selectedLanguage]) question.explanation_audio_urls[selectedLanguage] = {};
        question.explanation_audio_urls[selectedLanguage][params.voice] = data.url;
        
        if (selectedLanguage === "en") {
          if (params.voice === "female") question.female_explanation_audio_url = data.url;
          else question.male_explanation_audio_url = data.url;
        }
        ...
        const res = await fetch(`/api/admin/questions/${question.id}/voice?voice=${params.voice}&language=${selectedLanguage}`, {
          method: "DELETE"
        });
        const data = await res.json();
        if (!res.ok || !data?.success) throw new Error(data?.error || "Failed to delete audio");
        
        // Locally mutate `question` 
        if (question.explanation_audio_urls?.[selectedLanguage]) {
            delete question.explanation_audio_urls[selectedLanguage][params.voice];
        }
        if (selectedLanguage === "en") {
          if (params.voice === "female") question.female_explanation_audio_url = null;
          else question.male_explanation_audio_url = null;
        }
  ...
  return (
    <div className="p-1">
      ...
      {/* Language Selector (Visible only if on Female, Male, or Preview tab) */}
      {["female", "male", "preview"].includes(activeTab) && (
        <div className="mb-6 flex items-center justify-between bg-white dark:bg-gray-800 p-3 rounded-xl border border-gray-200 dark:border-gray-700 shadow-sm animate-fadeIn">
          <div className="flex items-center gap-2 text-sm font-medium text-gray-700 dark:text-gray-300">
          ...
          <select
            ...
            className="px-4 py-2 bg-gray-50 dark:bg-gray-900 border border-gray-200 dark:border-gray-700 rounded-lg text-sm font-medium outline-none focus:ring-2 focus:ring-blue-500 text-gray-700 dark:text-gray-200"
          >
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex gap-2">
                <button
                  ...
                  className="flex items-center gap-2 px-3 py-1.5 rounded-lg bg-gradient-to-r from-orange-500 to-orange-500 text-white  disabled:opacity-50 disabled:cursor-not-allowed transition-all duration-200 text-sm font-medium"
                >
                  <Sparkles size={14} />
  ...
  const handleVoice = async (voice, generateNew = false) => {
    try {
      ...
      if (existingUrl && !generateNew) {
        if (!audioRef.current) {
          // ensure an audio element exists in DOM to avoid AbortError on detach
          const el = new Audio();
          audioRef.current = el;
        }
    ...
    <div className="bg-white dark:bg-gray-800 rounded-2xl p-6 shadow-lg border border-gray-200 dark:border-gray-700 hover:shadow-xl transition-all duration-300">
      ...
      <div className="flex justify-between flex-wrap items-start mb-4">
        ...
        <div className="flex gap-2">
          <button
            onClick={onManage}
            className="flex items-center gap-1 text-orange-700 dark:text-orange-400 hover:bg-orange-50 dark:hover:bg-orange-900/20 px-3 py-1 rounded-lg transition-colors"
          ...
          <button
            ...
            className="flex items-center gap-1 text-gray-600 dark:text-gray-400 hover:text-orange-600 dark:hover:text-orange-400 px-3 py-1 rounded-lg hover:bg-orange-50 dark:hover:bg-orange-900/20 transition-colors"
          >
      ...
      {showExplanation && question.explanation && (
        <div className="mt-4 p-4 bg-orange-50 dark:bg-orange-900/20 rounded-lg border border-orange-200 dark:border-orange-800">
          ...
          <p className="text-orange-800 dark:text-orange-200 leading-relaxed whitespace-pre-line">
          ...
          <div className="mt-3 flex flex-wrap gap-2">
            <button
              ...
              className="flex items-center gap-2 text-orange-700 dark:text-orange-400 hover:bg-orange-50 dark:hover:bg-orange-900/20 px-3 py-1 rounded-lg disabled:opacity-60 transition-colors"
      ...
      {/* hidden audio element used for programmatic playback to avoid detaching issues */}
...
function TopicAccordion({
  ...
  return (
    <div className="bg-white dark:bg-gray-800 rounded-2xl shadow-lg border border-gray-200 dark:border-gray-700 hover:border-orange-300 dark:hover:border-orange-600 transition-all duration-300">
      {/* Topic Header */}
      <div
          ...
          {topic.questions?.length > 0 ? (
            ...
            <div className="text-center py-8 text-gray-500 dark:text-gray-400">
              ...
              <button
                ...
                className="mt-3 text-orange-600 dark:text-orange-400 hover:underline text-sm font-medium"
              >
          ...
          <div className="flex flex-col lg:flex-row lg:justify-between lg:items-start gap-6 mb-8">
            ...
            <div className="flex flex-col sm:flex-row gap-3 h-fit">
              <button
                ...
                className="flex items-center gap-3 bg-gradient-to-r from-orange-500 to-amber-600 hover:from-orange-600 hover:to-amber-700 text-white px-6 py-3 rounded-xl font-semibold transition-all duration-200 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5"
              >
                <Plus size={20} />
                <span>Add Topic</span>
              </button>
            ...
            <div className="flex flex-col lg:flex-row gap-4">
              ...
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  className="w-full pl-12 pr-4 py-3 border border-gray-200 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white placeholder-gray-500 dark:placeholder-gray-400 focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-200"
                />
              </div>
              ...
              <div className="flex gap-2">
                <select
                  ...
                  className="px-4 py-3 border border-gray-200 dark:border-gray-600 rounded-xl bg-white dark:bg-gray-800 text-gray-900 dark:text-white focus:ring-2 focus:ring-orange-500 focus:border-transparent transition-all duration-200"
                >
                ...
                <div className="text-center py-12 bg-white/80 dark:bg-gray-800/80 backdrop-blur-sm rounded-2xl shadow-sm border border-gray-200/50 dark:border-gray-700/50">
                  ...
                  {!(searchTerm || statusFilter !== "all") && (
                    <button
                      ...
                      className="flex items-center gap-2 bg-orange-500 hover:bg-orange-600 dark:bg-orange-600 dark:hover:bg-orange-500 text-white px-6 py-3 rounded-xl font-medium transition-colors duration-200 mx-auto"
                    >
                      <Plus size={20} />
