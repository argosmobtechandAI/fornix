...
export default function BrochureLeadsPage() {
  ...
  async function fetchLeads(f = filter) {
    ...
    try {
      const url = f === "all"
        ? "/api/admin/brochure-leads"
        : `/api/admin/brochure-leads?filter=${f}`;
  ...
  async function markRead(ids) {
    ...
    try {
      const res = await fetch("/api/admin/brochure-leads", {
        method: "PATCH",
  ...
  const readCount = leads.filter(l => l.is_read).length;
    ...
    <div className="min-h-screen bg-gray-50 dark:bg-gray-900 p-4 sm:p-6 w-full max-w-full">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Brochure Leads</h1>
            <p className="text-sm text-gray-500 dark:text-gray-400">Submissions for brochure downloads</p>
          </div>
        </div>
        <button
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 mb-6">
        {[
          { label: "Total leads", value: leads.length, color: "bg-blue-500", icon: FileText },
          { label: "Unread", value: unreadCount, color: "bg-orange-500", icon: EyeOff },
          { label: "Read", value: readCount, color: "bg-green-500", icon: Eye },
        ].map(stat => (
          <div key={stat.label} className="bg-white dark:bg-gray-800 rounded-2xl p-4 shadow-sm border border-gray-200 dark:border-gray-700 flex items-center gap-4">
            <div className={`${stat.color} w-12 h-12 rounded-xl flex items-center justify-center text-white shadow-lg shadow-${stat.color.split('-')[1]}-500/20`}>
              <stat.icon size={20} />
            </div>
            <div>
        ...
        <div className="flex flex-col lg:flex-row gap-4">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 border border-gray-200 dark:border-gray-700 rounded-xl bg-gray-50 dark:bg-gray-900 text-sm text-gray-900 dark:text-white placeholder-gray-400 focus:outline-none focus:ring-2 focus:ring-orange-500 transition-all shadow-inner"
            />
          </div>
      ...
      {loading ? (
        <div className="flex flex-col justify-center items-center py-32">
          ...
          <p className="text-gray-500 dark:text-gray-400 font-medium">Loading leads data...</p>
        </div>
      ) : filtered.length === 0 ? (
        <div className="text-center py-32 bg-white dark:bg-gray-800 rounded-3xl border border-gray-100 dark:border-gray-700 shadow-sm">
          ...
          <p className="text-gray-500 dark:text-gray-400">No brochure leads found.</p>
        </div>
      ) : (
            ...
            <div key={lead.id} className={`group bg-white dark:bg-gray-800 rounded-2xl shadow-sm border transition-all duration-300 flex flex-col relative ${
              ...
              <div className="p-6 flex-1">
                ...
                <div className="space-y-3 mb-6">
                  ...
                  <a href={`tel:${lead.phone}`} className="flex items-center gap-2 text-sm text-gray-600 dark:text-gray-400 hover:text-orange-500 transition-colors">
                    <Phone size={14} className="shrink-0" /> <span className="font-medium">{lead.country_code} {lead.phone}</span>
                  </a>
                </div>

                <div className="grid grid-cols-1 gap-3 p-4 bg-gray-50 dark:bg-gray-900/50 rounded-2xl border border-gray-100 dark:border-gray-700/50">
                  <div className="flex items-start gap-2.5">
                    <BookOpen size={16} className="text-orange-500 mt-0.5 shrink-0" />
                    <div>
                      <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-0.5">Interested In</p>
                      <p className="text-xs font-bold text-gray-900 dark:text-white line-clamp-2 leading-snug">{lead.courses?.name || "Unknown Course"}</p>
                    </div>
                  </div>
                  <div className="flex items-start gap-2.5">
                    ...
                    <div>
                      ...
                      <p className="text-xs font-bold text-gray-900 dark:text-white truncate">{lead.city || "—"}</p>
                    </div>
                  </div>
                </div>
              </div>
              ...
              <div className="p-4 bg-gray-50 dark:bg-gray-800/50 border-t border-gray-100 dark:border-gray-700 flex items-center justify-between">
                <button onClick={() => toggleOne(lead.id)} className="p-1 text-gray-300 hover:text-orange-500 transition-colors">
                  {selected.has(lead.id) ? <CheckSquare size={22} className="text-orange-600" /> : <Square size={22} className="text-gray-300" />}
                </button>
